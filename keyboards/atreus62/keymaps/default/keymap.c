...
#define LT_BSPC LT(_PN, KC_BSPC)
#define LT_LSPC LT(_DR, KC_SPC)
#define LT_RSPC LT(_FN, KC_SPC)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_BS] = LAYOUT( /* [> QWERTY <] */
    ...
    KC_ENT, KC_Z,   KC_X,   KC_C,   KC_V,   KC_B,                                 KC_N,   KC_M,   KC_COMM,KC_DOT, KC_SLSH,KC_BSLS,
            CPY_PST,C(KC_Z),KC_CAPS,LT_BSPC,LT_LSPC,LALT_T(KC_DEL),RALT_T(KC_ENT),LT_RSPC,LT_DEL, KC_PSCR,KC_MINS,KC_EQL,
  ...
  [_DR] = LAYOUT(
    ...
    XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,TG(_FN),                KC_BRID,KC_F1,  KC_F2,  KC_F3,  KC_F4,  KC_VOLD,
...
void post_process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case LT_BSPC:
        if (record->event.pressed) {
            if (IS_LAYER_ON(_FN)) backlight_enable();
        } else {
